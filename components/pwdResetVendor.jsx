...
import { vendorForgotPassword } from "@/lib/api/auth"
...
export default function PasswordResetVendor() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const RetData = await vendorForgotPassword({ emailId: email });
            var dataFromResponse = RetData.data;
            console.log(dataFromResponse)
            if (dataFromResponse.responseCode === 200) {
                toast({
                    title:  dataFromResponse.message ,
                    variant:"success"
                })
            }else{
                toast({
                    title:"the other toast",
                    variant:"destructive"
                })
            }
            setIsLoading(false)
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="text-center text-sm">
                            ...
                            <Link href="/vendor" className="text-[#B80D2D] hover:underline font-medium">
                                Sign in
                            </Link>
                        </div>
                    </CardContent>
